...
RSpec.describe 'POST /standup_meeting_groups/:standup_meeting_group_id/joins' do
  ...
  context 'when the standup meeting group user exists' do
    it 'does not create the standup meeting group user' do
      ...
      StandupMeetingGroupUser.create(user:, standup_meeting_group:)
      ...
      query = StandupMeetingGroupUser.where(user:, standup_meeting_group:)
